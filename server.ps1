...
try{while($listener.IsListening){$c=$listener.GetContext();$q=$c.Request;$r=$c.Response;$p=$q.Url.AbsolutePath
 try{
  ...
  if($p -eq '/api/state' -and $q.HttpMethod -eq 'POST'){$raw=Read-RawBody $q;if($raw){[IO.File]::WriteAllText((Join-Path $root 'vyra-state-backup.json'),$raw,$utf8NoBom)};Send-Json $r @{ok=$true};continue}
