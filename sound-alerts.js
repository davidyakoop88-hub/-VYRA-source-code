...
function aeRead(){return JSON.parse(localStorage.getItem(AE_KEY)||'{"actions":[],"events":[]}')}
function aeWrite(state){localStorage.setItem(AE_KEY,JSON.stringify(state))}
...
function soundAlertCard(sound){let conn=saConnection(sound.id),triggerOptions=Object.entries(SA_TRIGGERS).map(([k,v])=>`<option value="${k}">${v}</option>`).join('');return `<article class="media-card sound-card" data-sound="${sound.id}"><div class="sound-icon"><i>♪</i><button class="media-play" type="button">▶</button></div><div class="media-info"><b>${sound.name}</b>${conn?`<small>Kopplad till: ${SA_TRIGGERS[conn.event.trigger]||conn.event.trigger}</small><button class="use-media sa-disconnect" type="button">Koppla från</button>`:`<small>Ej kopplad</small><label class="sa-trigger-row">Trigger<select class="sa-trigger">${triggerOptions}</select></label><input class="sa-condition" placeholder="Villkor (valfritt, t.ex. giftnamn)"><button class="use-media sa-connect" type="button">Koppla till event</button>`}</div></article>`}

function soundAlertsHtml(){let ids=Object.keys(soundAlerts);if(!ids.length)return `<div class="section-head"><div><h2>Sound Alerts</h2><p>0 ljudklipp</p></div></div><p class="sa-empty">Inga ljud tillagda ännu — skicka ljudklipp så läggs de till här, precis som Overlay-paket.</p>`;return `<div class="section-head"><div><h2>Sound Alerts</h2><p>${ids.length} ljudklipp · koppla till events</p></div></div><div class="media-grid">${ids.map(id=>soundAlertCard(soundAlerts[id])).join('')}</div>`}
