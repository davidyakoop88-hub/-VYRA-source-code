(function () {
  ...
  wh = function (w) {
    ...
    return `<div class="widget follower-spotlight${selected === w.id ? ' selected' : ''}" data-id="${w.id}" style="left:${w.x}px;top:${w.y}px;width:${w.width || 300}px;--follow:${w.followColor || Object.values(cfg.themes)[0]};zoom:${w.widgetScale || 1}"><div class="follow-light"></div><div class="follow-avatar"><img src="${w.profileImage || 'assets/images/e16771233251580175db2a7388b9acc4.jpg'}"></div><div class="follow-label"><i></i><span>${w.followLabel || cfg.label}</span><i></i></div><h2>${w.followName || cfg.defaultName}</h2><p>${w.followMessage || cfg.defaultMsg}</p>${selected === w.id ? '<span class="resize-handle">↘</span>' : ''}</div>`;
  };
